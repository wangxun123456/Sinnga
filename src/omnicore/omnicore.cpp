...
static bool FillTxInputCache(const CTransaction& tx)
{
    ...
    for (std::vector<CTxIn>::const_iterator it = tx.vin.begin(); it != tx.vin.end(); ++it) {
        ...
        COutPoint out(txIn.prevout.hash, nOut);
...
static int parseTransaction(bool bRPConly, const CTransaction& wtx, int nBlock, unsigned int idx, CMPTransaction& mp_tx, unsigned int nTime)
{
    ...
    if (omniClass != OMNI_CLASS_C)
    {
        ...
        for (unsigned int i = 0; i < wtx.vin.size(); ++i) {
            ...
            const CTxOut& txOut = view.AccessCoin(txIn.prevout).out;
...
static int msc_initial_scan(int nFirstBlock)
{
    ...
    bool seedBlockFilterEnabled = gArgs.GetArg("-omniseedblockfilter", true);
...
int mastercore_init()
{
    ...
    if (!gArgs.GetArg("-autocommit", true)) {
    ...
    if (gArgs.GetArg("-startclean", false)) {
...
bool mastercore::UseEncodingClassC(size_t nDataSize)
{
    ...
    bool fDataEnabled = gArgs.GetArg("-datacarrier", true);
...
{
    if (isNonMainNet()) {
        return DecodeDestination(exodus_testnet);
    } else {
        return DecodeDestination(exodus_mainnet);
    }
}
...
const CTxDestination ExodusCrowdsaleAddress(int nBlock)
{
    if (MONEYMAN_TESTNET_BLOCK <= nBlock && isNonMainNet()) {
        return DecodeDestination(getmoney_testnet);
    }
