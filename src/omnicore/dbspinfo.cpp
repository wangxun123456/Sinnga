...
bool CMPSPInfo::updateSP(uint32_t propertyId, const Entry& info)
{
    ...
    ssSpValue.reserve(GetSerializeSize(ssSpValue, info));
    ssSpValue << info;
...
uint32_t CMPSPInfo::putSP(uint8_t ecosystem, const Entry& info)
{
    ...
    ssTxValue.reserve(GetSerializeSize(ssSpValue, propertyId));
...
void CMPSPInfo::setWatermark(const uint256& watermark)
{
    ...
    ssValue.reserve(GetSerializeSize(ssValue, watermark));
    ssValue << watermark;
