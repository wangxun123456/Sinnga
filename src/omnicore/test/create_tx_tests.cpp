...
BOOST_AUTO_TEST_CASE(txbuilder_add_change)
{
    ...
    CMutableTransaction tx = TxBuilder()
        ...
        .addOutput(GetScriptForDestination(DecodeDestination("174TgzbFFWiKg1VWt8Z55EVP7rJ54jQSar")), 150000000LL)
    ...
    tx = TxBuilder(tx)
        .addChange(DecodeDestination("12gxzZL9g6tWsX6ut8srcgcUTQ4c9wWuGS"), viewTemp, 13242LL)
        .build();
...
BOOST_AUTO_TEST_CASE(txbuilder_add_change_position)
{
    ...
    CMutableTransaction tx = TxBuilder(txBasis)
        .addChange(DecodeDestination("1DJFjEV9U7TgyDZVT1tcCGJDhDeRYSQGuD"), viewTemp, 50000LL, 1)
        .build();
