...
CScript scriptPubKey = GetScriptForDestination(DecodeDestination(destination));
...
return (OmniTxBuilder&) TxBuilder::addChange(DecodeDestination(destination), view, txFee, position);
}
...
void InputsToView(const std::vector<PrevTxsEntry>& prevTxs, CCoinsViewCache& view)
{
    ...
    for (std::vector<PrevTxsEntry>::const_iterator it = prevTxs.begin(); it != prevTxs.end(); ++it) {
	    ...
	    if(!view.HaveCoinInCache(it->outPoint)) {
			view.AddCoin(it->outPoint, Coin(std::move(it->txOut), 1, false), true);
		}
    }
}
